...
class Chatboot:
    ...
    def response_ai(self, model_name, query, vector_store):
        # Template do prompt com o contexto em português
        system_prompt = '''
        ...
        # Criação do histórico de mensagens
        ...
        # Criação dos chains usando Hugging Face
        llm = ChatHuggingFace(model=model_name)  # Modelo de linguagem da Hugging Face em português
        ...
        if upload_documents:
            with st.spinner('Processando documentos...'):
                ...
                for document in upload_documents:
                    chunks = self.__chunks_generate(file=document)
    ...
    def load_vector_store(self):
        if os.path.exists(self.repository_db):
            ...
            vector_store = Chroma(
                ...
                embedding_function=HuggingFaceEmbeddings()  # Alterado para embeddings da Hugging Face
            )
            return vector_store
        return None
    ...
    def __chunks_generate(self, file):
        temp_file_path = None
